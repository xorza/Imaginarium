...
fn main() {
    ...
    let ctx = Gpu::new_for_internals().expect("Failed to create GPU context");
    ...
    let input = GpuImage::from_image_for_internals(&ctx, &input_cpu);
    ...
    // Scale down 50%
    let mut output = GpuImage::new_empty_for_internals(
        &ctx,
    ...
    let output_cpu = output.to_image_for_internals(&ctx).unwrap();
    ...
    let mut output = GpuImage::new_empty_for_internals(&ctx, input_cpu.desc());
    ...
    let output_cpu = buffer_a.to_image_for_internals(&ctx).unwrap();
    save_image(&output_cpu, "transform_multi_pass.png");

    // Compare filter modes: nearest vs bilinear on upscale
    let mut output_nearest = GpuImage::new_empty_for_internals(
        &ctx,
    ...
    let output_cpu = output_nearest.to_image_for_internals(&ctx).unwrap();
    save_image(&output_cpu, "transform_upscale_nearest.png");

    let mut output_bilinear = GpuImage::new_empty_for_internals(
        &ctx,
        ImageDesc::new(1024, 1024, input_cpu.desc().color_format),
    );
    Transform::new()
    ...
    let output_cpu = output_bilinear.to_image_for_internals(&ctx).unwrap();

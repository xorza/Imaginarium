...
5. **Ops** — `Blend`, `ContrastBrightness`, `Transform`; each exposes an `execute(&mut ProcessingContext, …)` that calls `select_backend` then routes to a CPU or GPU path. Plus `Preview` — a CPU-only convenience op outside that pattern (`Preview::new(w,h).to_rgba8(&Image) -> Image`): a fused area-average downscale + `RGBA_U8` convert for fast thumbnails.
...
`ImageBuffer` (`image_buffer.rs:28`) is the CPU/GPU residency abstraction: `{ desc, storage: RwLock<Option<Storage>> }` where `Storage` is `Cpu(Image)` | `Gpu(GpuImage)`. The interior mutability lets `make_cpu()`/`make_gpu()` (and `make_*_mut`) upgrade residency in place — uploading via `GpuImage::from_image` or downloading via `GpuImage::to_image` only when the target side is missing. Resident readers share the lock; a residency change waits for them to finish. `to_cpu`/`to_gpu` consume and transfer. **This is the join point between the op layer and the GPU layer**: ops call `make_gpu`/`make_cpu` on their buffers as the chosen backend requires.
...
The backend-routed ops (a fourth, `Preview`, is CPU-only — see layer 5 above):
...
| Transform | `Transform { transform: Affine2, filter: FilterMode }`, `FilterMode { Nearest, Bilinear }` (default Bilinear); builders `scale`/`rotate`/`rotate_around`/`translate`/`affine`/`filter` (`transform/mod.rs:39`) | rayon-parallel scalar `apply_typed::<T, N>` — inverse-maps each output pixel and nearest/bilinear-samples; all 9 formats, cross-checked against the GPU | `GpuTransformPipeline`, applies `Affine2` + interpolation | `shader.wgsl` |

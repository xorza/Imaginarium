...
/// One row of `format`, filled with the pattern its storage type spans.
fn source_row(format: ColorFormat, width: usize) -> Vec<u8> {
    let row = match (format.channel_size, format.channel_type) {
        (ChannelSize::_8bit, ChannelType::UInt) => create_test_image(width, 1, format),
        (ChannelSize::_16bit, ChannelType::UInt) => create_test_image_u16(width, 1, format),
        (ChannelSize::_32bit, ChannelType::Float) => create_test_image_f32(width, 1, format),
...
/// moment it is added, with no new test to remember.
#[test]
    ...
    for &from in ALL_FORMATS {
        for &to in ALL_FORMATS {
            ...
            for &width in &TEST_WIDTHS {
                ...
                let mut vector = vec![0u8; width * to.byte_count() as usize];
                let mut scalar = vec![0u8; vector.len()];

                // SAFETY: `row_converter` verified this CPU has the feature the
                // kernel it handed back was compiled for.
    ...
    for &width in &TEST_WIDTHS {
        ...
        for y in 0..2 {
            ...
            for x in 0..width {
                ...
                assert_eq!(src_r, dst_r, "R mismatch at ({x}, {y}) width={width}");
                assert_eq!(src_g, dst_g, "G mismatch at ({x}, {y}) width={width}");
                assert_eq!(src_b, dst_b, "B mismatch at ({x}, {y}) width={width}");
            }
        }
    }
}
            ...
            for x in 0..width {
                for c in 0..4 {
                    assert_eq!(
                        ...
                        "Round-trip mismatch at ({x}, {y})[{c}] width={width}"
                    );
                }
            }
        }
        ...
        for y in 0..2 {
            ...
            for x in 0..width {
                ...
                assert!(
                    ...
                    "Luminance mismatch at ({x}, {y}) width={width}: expected {expected}, got {actual}"
                );
            }
        }
    }
            ...
            for x in 0..width {
                for c in 0..4 {
                    assert_eq!(
                        src_bytes[src_row + x * 4 + c],
                        back_bytes[back_row + x * 4 + c],
                        "U8->U16->U8 round-trip failed at ({x}, {y})[{c}] width={width}"
                    );
                }
            }
        }
        ...
        for y in 0..2 {
            ...
            for x in 0..width {
                for c in 0..4 {
                    ...
                    assert!(
                        (expected as i32 - actual as i32).abs() <= 1,
                        "F32->U8 mismatch at ({x}, {y})[{c}] width={width}: expected {expected}, got {actual}"
                    );
                }
            }
        }
        ...
        for y in 0..2 {
            for x in 0..width {
                ...
                assert!(
                    ...
                    "L_U16->F32 mismatch at ({x}, {y}) width={width}: expected {expected}, got {f32_val}"
                );
            }
        }
    }
        ...
        for y in 0..2 {
            for x in 0..width {
                ...
                assert!(
                    ...
                    "L_F32->U16 mismatch at ({x}, {y}) width={width}: expected {expected}, got {u16_val}"
                );
            }
        }
    }
    ...
    for (&fmt_u8, &fmt_u16) in formats_u8.iter().zip(formats_u16.iter()) {
        ...
        for i in 0..src_bytes.len().min(back_bytes.len()) {
            assert_eq!(
                ...
                "Round-trip failed for {fmt_u8:?} <-> {fmt_u16:?} at byte {i}"
            );
        }
    }
}

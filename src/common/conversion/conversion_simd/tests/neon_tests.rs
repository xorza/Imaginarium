    ...
    for &width in &TEST_WIDTHS {
        ...
        for x in 0..width {
            assert_eq!(
                dst[x * 3],
                src[x * 4],
                "R mismatch at x={x} width={width}"
            );
            assert_eq!(
                dst[x * 3 + 1],
                src[x * 4 + 1],
                "G mismatch at x={x} width={width}"
            );
            assert_eq!(
                dst[x * 3 + 2],
                src[x * 4 + 2],
                "B mismatch at x={x} width={width}"
            );
        }
    }
}
...
fn test_neon_u8_to_u16_various_widths() {
    for &width in &TEST_WIDTHS {
        ...
        for i in 0..src.len() {
            ...
            assert_eq!(
                dst[i], expected,
                "U8->U16 mismatch at i={i} width={width}"
            );
        }
    }
}
...
fn test_neon_u8_u16_round_trip() {
    for &width in &TEST_WIDTHS {
        ...
        assert_eq!(
            src, back,
            "U8->U16->U8 round-trip failed at width={width}"
        );
    }
}
...
fn test_neon_simd_boundary_widths() {
    ...
    for &width in &widths {
        ...
        assert_eq!(
            dst[last * 3],
            src[last * 4],
            "Last pixel R at width={width}"
        );
        assert_eq!(
            dst[last * 3 + 1],
            src[last * 4 + 1],
            "Last pixel G at width={width}"
        );
        assert_eq!(
            dst[last * 3 + 2],
            src[last * 4 + 2],
            "Last pixel B at width={width}"
        );
    }
}

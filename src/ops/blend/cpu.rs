...
mod tests {
    ...
    fn test_normal_blend_alpha_zero() {
        ...
        for format in ALL_FORMATS {
            ...
            assert!(
                diff < 1e-6,
                "alpha=0 should return dst for format {format}, got diff={diff}"
            );
        }
    }
    ...
    fn test_normal_blend_alpha_one() {
        ...
        for format in ALL_FORMATS {
            ...
            assert!(
                diff < 0.01,
                "alpha=1 Normal should return src for format {format}, got diff={diff}"
            );
        }
    }
    ...
    fn test_multiply_with_white() {
        ...
        assert!(
            max_rgb_diff < 0.01,
            "Multiply with white should return dst RGB, got diff={max_rgb_diff}"
        );
    }
        ...
        for format in ALL_FORMATS {
            for mode in &modes {
                ...
                assert!(
                    ...
                    "Blend {mode:?} failed for format {format}"
                );
            }
        }
    }
